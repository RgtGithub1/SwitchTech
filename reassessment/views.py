...
def reassessmentquiz(request):
    try:
        if request.method == 'POST':
            ...
            if total_user_quiz_score >= 60:

                context = {'user_name': user, 'time': current_time, 'score': total_user_quiz_score, 'text1':'Congratulations! You have successfully passed the quiz.'}

                return render(request, 'reassessment_result.html', context)
            else:
                context = {'user_name': user, 'time': current_time, 'score': total_user_quiz_score, 'text2':'I regret to inform you that you did not successfully pass the quiz. You are welcome to retake the quiz to improve your results'}

                return render(request, 'reassessment_result.html', context)
    except Exception as e:
        return HttpResponse('Their are no questions')